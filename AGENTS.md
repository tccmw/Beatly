...
- Crash: `a/5`, Voice 1, `x` notehead.
- Ride: `f/5`, Voice 1, `x` notehead.
- Tom: `e/5`, Voice 1, normal notehead.
- 손 성부는 Voice 1, stem up으로 유지한다.
- 발 성부는 Voice 2, stem down으로 유지한다.
- 손 성부의 cymbal(`x`)은 크래시/하이햇/라이드별 표준 lane에 둔다.
- 손 성부의 drum(normal)은 탐/스네어별 표준 lane에 둔다.
- 같은 슬롯에 cymbal과 drum이 함께 있어도 같은 Y로 붕괴되면 안 된다.
...
- Common harness: all-instrument shared rules.
- Instrument harness: per-instrument backend/frontend mapping rules.
